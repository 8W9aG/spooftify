/*
 * $Id: xml.c 520 2012-03-17 10:04:59Z dalus $
 *
...
struct playlist* xml_parse_playlist(struct playlist* pl,
                                    ...
                                    bool list_of_lists)
{
    ezxml_t top = ezxml_parse_str((char*)xml, len);
    ...
    if (list_of_lists) {
        ...
        for (char* id = strtok(items, ",\n"); id; id = strtok(NULL, ",\n"))
        {
            ...
            DSFYstrncpy((char*)p->playlist_id, id, sizeof p->playlist_id);
            prev = p;
        }
    }
    else {
        ...
        for (char* id = strtok(items, ",\n"); id; id = strtok(NULL, ",\n"))
        {
            ...
            if (prev)
                ...
                root = t;
            DSFYstrncpy((char*)t->track_id, id, sizeof t->track_id);
            prev = t;
            track_count++;
        }
        pl->tracks = root;
    ...
    for (ezxml_t track = ezxml_get(xml, "track",-1); track; track = track->next)
    {
        ...
        if (ordered) {
            ...
            for (tt = root; tt; tt = tt->next)
                if (!tt->has_meta_data &&
                    !strncmp((const char*)tt->track_id, tid, sizeof tt->track_id))
                    break;
            ...
            if (!tt) {
                ...
                for (ezxml_t re = ezxml_child(track, "redirect"); re; re = re->next) {
                    ...
                    for (tt = root; tt; tt = tt->next) {
                        ...
                        if (!tt->has_meta_data &&
                            !strncmp((const char*)tt->track_id, rid, sizeof tt->track_id)) {
        ...
        xmlstrncpy((char*)t->track_id, sizeof t->track_id, track, "id", -1);
        xmlstrncpy((char*)t->cover_id, sizeof t->cover_id, track, "cover", -1);
        xmlstrncpy((char*)t->album_id, sizeof t->album_id, track, "album-id", -1);
        ...
        for ( ezxml_t file = ezxml_get(track, "files",0, "file",-1); file; file = file->next) {
            ...
            if (fmt) {
                ...
                if (id) {
                    DSFYstrncpy((char*)t->file_id, id, sizeof t->file_id);
                    t->playable = true;
                }
            }
        }
...
static void parse_browse_album(ezxml_t top, struct album_browse* a, bool high_bitrate)
{
    ...
    for (struct track *t = a->tracks; t; t = t->next) {
        ...
        DSFYstrncpy((char*)t->album_id, a->id, sizeof t->album_id);
        DSFYstrncpy((char*)t->cover_id, (char*)a->cover_id, sizeof t->cover_id);
        t->year = a->year;
        count++;
    }
}
                     ...
                     bool high_bitrate)
{
    ...
    xmlstrncpy((char*)search->suggestion, sizeof search->suggestion,
               top, "did-you-mean", -1);
...
void xml_parse_prodinfo(struct user_info* u, unsigned char* xml, int len)
{
    ...
    unsigned int expiry;
    xmlatoi((int*)&expiry, top, "product", 0, "expiry", -1);
    u->expiry = expiry;
    ezxml_free(top);
}
