    ...
    case CHANNEL_END:
            ...
            if (ch->total_data_len < SUBSTREAM_SIZE) {
                ...
                if (ds->track && ds->play_as_list) {
                    char fid[20], tid[16];
                    hex_ascii_to_bytes((char*)ds->track->file_id, (unsigned char*)fid, sizeof fid);
                    hex_ascii_to_bytes((char*)ds->track->track_id, (unsigned char*)tid, sizeof tid);
                    
                    /* request key for next track */
                    error = cmd_aeskey(ds->session, (unsigned char*)fid, (unsigned char*)tid, despotify_aes_callback, ds);
                }
            }
...
int despotify_snd_read_stream(struct despotify_session* ds)
{
    ...
    if (cmd_getsubstreams(ds->session, (unsigned char*)fid,
                          ds->offset, SUBSTREAM_SIZE,
                    ...
                    struct track* t, bool play_as_list)
{
    ...
    int error = cmd_aeskey(ds->session, (unsigned char*)fid, (unsigned char*)tid, despotify_aes_callback, ds);
    if (error) {
                                       ...
                                       char* searchtext, int maxresults)
{
    ...
    if (b) {
        ...
        DSFYstrncpy((char*)search->query, searchtext, sizeof search->query);
                                            ...
                                            int offset, int maxresults)
{
    ...
    int ret = cmd_search(ds->session, (char*)search->query,
                         offset, maxresults,
                         despotify_gzip_callback, ds);
    if (ret) {
        ds->last_error = "cmd_search() failed";
...
static bool despotify_load_tracks(struct despotify_session *ds, bool cache_do_store)
{
    ...
    for (int totcount=0; totcount < pl->num_tracks; totcount += count) {
        ...
        for (count = 0; t && count < MAX_BROWSE_REQ; t = t->next, count++)
            hex_ascii_to_bytes((char*)t->track_id, (unsigned char*)(tracklist + count * 16), 16);

        /* create a hash of the tracklist */
        SHA1Init(&sha_ctx);
        SHA1Update(&sha_ctx, (const unsigned char*)tracklist, count * 16);
        SHA1Final(hash, &sha_ctx);

        hex_bytes_to_ascii(hash, (char*)tracks_hash, 20);
        ...
        if (ds->use_cache && cache_contains(tracks_hash)) {
            ...
            if ((data = cache_load(tracks_hash, (unsigned int*)&len)) != NULL) {
        ...
        int error = cmd_browse(ds->session, BROWSE_TRACK, (unsigned char*)tracklist, count,
                               despotify_gzip_callback, ds);
        ...
        for (t = pl->tracks; t; t = t->next) {
            if (!t->has_meta_data) {
                ...
                for (tt = pl->tracks; tt; tt = tt->next) {
                    if (tt->has_meta_data &&
                        !strncmp((const char*)tt->track_id, (const char*)t->track_id, sizeof tt->track_id)) {
                                        ...
                                        char* playlist_id, bool cache_do_store)
{
    ...
    /* check cache */
    if (ds->use_cache && playlist_id && cache_contains((unsigned char*)playlist_id)) {
        unsigned char* data;
        int len;

        DSFYDEBUG("Loading cached playlist '%s'...\n", playlist_id);

        if ((data = cache_load((unsigned char*)playlist_id, (unsigned int*)&len)) != NULL) {
            ds->playlist = xml_parse_playlist(ds->playlist, data, len, false);
            free(data);

            DSFYstrncpy((char*)ds->playlist->playlist_id, playlist_id, sizeof ds->playlist->playlist_id);
    ...
    char pid[17];
    if (playlist_id)
        hex_ascii_to_bytes(playlist_id, (unsigned char*)pid, 17);
    else {
    ...
    int error = cmd_getplaylist(ds->session, (unsigned char*)pid, PLAYLIST_CURRENT,
    ...
    if (cache_do_store && playlist_id)
        cache_store((unsigned char*)playlist_id, ds->response->ptr, ds->response->len);
...
struct playlist* despotify_get_stored_playlists(struct despotify_session *ds)
{
    ...
    for (struct playlist* p = metalist; p && p->playlist_id[0]; p = p->next) {
        ...
        struct playlist* new = despotify_get_playlist(ds, (char*)p->playlist_id, old_use_cache);
        if (prev)
            prev->next = new;
        else
            root = new;
                               ...
                               struct playlist *playlist, char *name)
{
    ...
    hex_ascii_to_bytes((char*)playlist->playlist_id, pid, 17);
                                           ...
                                           char* artist_id)
{
    ...
    /* check cache */
    if (ds->use_cache && cache_contains((unsigned char*)artist_id)) {
        unsigned char* data;
        int len;

        DSFYDEBUG("Loading cached artist '%s'...\n", artist_id);

        if ((data = cache_load((unsigned char*)artist_id, (unsigned int*)&len)) != NULL) {
    ...
    if (b) {
        ...
        if (ds->use_cache)
            cache_store((unsigned char*)artist_id, b->ptr, b->len);
...
void* despotify_get_image(struct despotify_session* ds, char* image_id, int* len, bool only_cache)
{
    ...
    if (ds->use_cache)
        cache_store((unsigned char*)image_id, ds->response->ptr, ds->response->len);
                                         ...
                                         char* album_id)
{
    ...
    /* check cache */
    if (ds->use_cache && cache_contains((unsigned char*)album_id)) {
        unsigned char* data;
        int len;

        DSFYDEBUG("Loading cached album '%s'...\n", album_id);

        if ((data = cache_load((unsigned char*)album_id, (unsigned int*)&len)) != NULL) {
...
struct track* despotify_get_tracks(struct despotify_session* ds, char* track_ids[], int num_tracks)
{
    ...
    for (int i = 0; i < num_tracks; i++)
        hex_ascii_to_bytes(track_ids[i], (unsigned char*)(tracklist + i * 16), 16);

    int error = cmd_browse(ds->session, BROWSE_TRACK, (unsigned char*)tracklist, num_tracks,
                           despotify_gzip_callback, ds);
...
char* despotify_playlist_to_uri(struct playlist* playlist, char* dest)
{
    ...
    strncpy(id, (const char*)playlist->playlist_id, 32);
    id[32] = '\0';
...
char* despotify_track_to_uri(struct track* track, char* dest)
{
    ...
    despotify_id2uri((char*)track->track_id, uri);
