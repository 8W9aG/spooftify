...
void auth_generate_auth_hash (SESSION * session)
{
	...
	SHA1Update (&ctx, (const unsigned char*)session->salt, 10);
	SHA1Update (&ctx, (const unsigned char*)" ", 1);
	SHA1Update (&ctx, (const unsigned char*)session->password, strlen (session->password));
